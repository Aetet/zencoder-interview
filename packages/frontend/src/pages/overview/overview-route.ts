...
export const overviewRoute = reatomRoute({
  ...
  const INITIAL = {
    summary: {
      ...
      costPerSession: 0,
      totalCost: 0,
      trend: [] as DailySessionTrend[],
      costTrend: [] as DailyCostTrend[],
    },
    insights: [] as Insight[],
    teams: [] as Team[],
  ...
  const completionRate = computed(
    () => liveKpis()?.completionRate ?? data().summary.completionRate,
  ...
  const costPerSession = computed(
    () => liveKpis()?.costPerSession ?? data().summary.costPerSession,
